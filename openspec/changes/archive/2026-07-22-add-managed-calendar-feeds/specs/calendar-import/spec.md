...
Each saved feed SHALL record when it was last synchronised and what happened — how many events were created, skipped, hidden, and flagged for review, or the reason the synchronisation failed. The backend SHALL show this alongside the feed.

#### Scenario: Successful synchronisation is recorded
- **WHEN** a feed is synchronised successfully
- **THEN** the system SHALL record the time and the counts of created, skipped, hidden, and flagged events, and SHALL display them with the feed

#### Scenario: Failed synchronisation is recorded and visible
- **WHEN** synchronising a feed fails because its URL is unreachable or no longer valid
- **THEN** the system SHALL record the failure and its reason against that feed, and SHALL show it in the backend so the feed is not silently broken
