...
- **Source of truth (Option A, with a build refinement):** S3 is the runtime single source of truth — the running app reads and writes content to the S3 content bucket. The **build** is the one exception: the Amplify build container has no S3 credentials, so `next build` reads the committed git `content/` seed instead (guarded by `NEXT_PHASE === "phase-production-build"` in `getStore()`). This is freshness-safe because the seed equals the S3 state at deploy time (we `aws s3 sync content/` before connecting), and time-based ISR (below) reconciles any post-deploy edits from S3 within the window. The git `content/` directory therefore serves a dual role: the build's prerender source **and** the one-time S3 seed; it never clobbers live S3 writes (runtime always reads S3).
