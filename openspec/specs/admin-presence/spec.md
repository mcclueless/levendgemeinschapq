...
When an administrator hides or deletes an item from its own public page, the system SHALL apply the same content rules as the backend and SHALL return the administrator to the relevant public listing (since the item's own page is no longer publicly available), with a confirmation of the outcome. Hiding a venue or organiser that is still referenced by a published event or blog post SHALL be blocked using the existing reference guard; the item SHALL remain published and the administrator SHALL be shown which published items still reference it.
...
#### Scenario: Deleting an event from its page
- **WHEN** an administrator confirms "Verwijderen" on an event page
- **THEN** the system SHALL permanently delete the event, revalidate the public site, and redirect to the public events listing with a confirmation
...
- **THEN** the system SHALL NOT hide it and SHALL show a message naming the referencing published items, mirroring the backend's reference guard
